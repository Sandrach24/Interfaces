...
class Emprendimiento(models.Model):
    ...
    foto= models.ImageField(upload_to="fotoNegocio/", verbose_name="Foto del Negocio", null = True, blank=True)
    video= models.URLField(verbose_name="Video del emprendimiento",blank=True, null=True)
    ...
    productos= models.ManyToManyField(Producto, verbose_name='Productos', related_name='producto')
    
    def average_rating(self) -> float:
        return Rating.objects.filter(emprendimiento=self).aggregate(Avg("rating"))["rating__avg"] or 0
    
    def __str__(self):
        return f"{self.nombreEmprendimiento}: {self.average_rating()}"
...
class Rating(models.Model):
    ...
    def __str__(self):
        return f"{self.emprendimiento.nombreEmprendimiento}: {self.rating}"
